...
// Handle form submission for editing expenses
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['expense_id'])) {
    ...
    $update_sql = "UPDATE expenses SET category='$category', amount='$amount', payment_method='$payment_method' WHERE id='$expense_id'";
    if ($conn->query($update_sql) === TRUE) {
        ...
        echo "<p class='error-msg'>Error updating expense: " . $conn->error . "</p>";
    }
}
...
<div class="sidebar">
    ...
    <div class="main-content">
        ...
        <?php if ($success_message): ?>
            <p class="success-msg"><?php echo htmlspecialchars($success_message); ?></p>
        <?php endif; ?>
                ...
                <?php while($row = $result->fetch_assoc()): ?>
                    <div class="expense-item">
                        <form method="POST" action="">
                            ...
                            <label for="category_<?php echo $row['id']; ?>">Category:</label>
                            <select id="category_<?php echo $row['id']; ?>" name="category">
                                <option value="Groceries" <?php echo $row['category'] === 'Groceries' ? 'selected' : ''; ?>>Groceries</option>
                                ...
                                <option value="Transportation" <?php echo $row['category'] === 'Transportation' ? 'selected' : ''; ?>>Transportation</option>
                                <option value="Entertainment" <?php echo $row['category'] === 'Entertainment' ? 'selected' : ''; ?>>Entertainment</option>
                            </select>
                            <label for="payment_method_<?php echo $row['id']; ?>">Payment Method:</label>
                            <select id="payment_method_<?php echo $row['id']; ?>" name="payment_method">
                            ...
                            <label for="amount_<?php echo $row['id']; ?>">Amount:</label>
                            <input type="number" id="amount_<?php echo $row['id']; ?>" name="amount" step="0.01" value="<?php echo htmlspecialchars($row['amount']); ?>" required>
